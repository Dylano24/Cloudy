...
if (!text.includes('const logoMediaRow = new ActionRowBuilder().addComponents(')) {
    ...
    // These four controls intentionally share one Discord ActionRow. On the
    // narrow mobile layout Discord wraps them into the two requested visible
    // lines: logo controls first, media controls second.
    ...
    // Same principle here: mobile wraps this ActionRow into footer/color and
    // edit/remove-buttons, matching the requested seven-line visual layout.
...
console.log(`[EMBED_BUILDER_SEVEN_VISIBLE_ROWS] ${text === before ? 'already current' : 'prepared one-message seven visible rows'}`);

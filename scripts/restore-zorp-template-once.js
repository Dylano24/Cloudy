...
client.once('ready', async () => {
  try {
    ...
    const embeds = target.embeds.map((embed, i) => i === index ? new EmbedBuilder(restoredData) : new EmbedBuilder(embed.toJSON()));
    const edited = await target.edit({ embeds });
    const saved = await saveEmbedTemplateDecoration(guild.id, channel.id, ['☑️ ZORP Guide', 'ZORP Guide'], restoredData, { applyFields: true, applyThumbnail: false, applyImage: false });
    if (!saved) throw new Error('Template persistence failed');
    console.log(`[ZORP_RESTORE] RESTORE_ZORP_AND_TEMPLATE_OK message=${edited.id} channel=${channel.id}`);
  } catch (error) {

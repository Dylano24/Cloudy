...
function payload(state, result = null, ended = false) {
  const file = createCardVisual({
    filename: `blackjack-${state.id}-${state.deck.length}.svg`,
    rows: [
      ...state.hands.map((hand, index) => ({ label: state.hands.length > 1 ? `Hand ${index + 1}` : 'Your hand', cards: hand.cards })),
      { label: 'Dealer', cards: state.finished ? state.dealer : [state.dealer[0], null] },
    ],
  });
  const gameEmbed = embed(state, result).setImage(`attachment://${file.name}`);
  return { embeds: [gameEmbed], components: controls(state, ended), files: [file] };
}
  ...
  execute: withErrorHandling(async (interaction, config, client) => {
    ...
    collector.on('collect', async component => {
      ...
      try {
        ...
        if (hand.done && state.current < state.hands.length - 1) { state.current += 1; await component.update(payload(state)); return; }
